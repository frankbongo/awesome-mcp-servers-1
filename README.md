...
- **[Disc Golf MCP Suite](https://github.com/discgolfapi)** - Remote Streamable HTTP suite for disc golf: course search (6,500+ global courses via DiscGolfAPI), disc flight comparisons & weekly sales rankings (TheDiscList), and bag gap analysis with visual Bag Map deep linking (BagIQ). Free, public, read-only, no API key required.
